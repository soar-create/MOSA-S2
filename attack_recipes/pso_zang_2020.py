...
class PSOZang2020(AttackRecipe):
    ...
    def build(model_wrapper):
        ...
        #加上maximizable=True就可以输出结果不管是否更改标签
        goal_function = UntargetedClassification(model_wrapper, maximizable=True)
        #
        # Perform word substitution with a Particle Swarm Optimization (PSO) algorithm.
        #
        #search_method = ParticleSwarmOptimization(pop_size=20, max_iters=10)
        #search_method = GreedyWordSwapWIR(wir_method="random")
        #search_method = BeamSearch(beam_width=10)
        search_method = AlzantotGeneticAlgorithm(pop_size=38, max_iters=12, post_crossover_check=False)
        #search_method = ImprovedGeneticAlgorithm(
         #   pop_size=30,
          #  max_iters=10,
           # max_replace_times_per_index=10,

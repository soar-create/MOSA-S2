...
class AnnealingGeneticAlgorithm(PopulationBasedSearch, ABC):
    ...
    def crossover_operation(self, pop_member1, pop_member2, T, original_text):
        ...
        for i in range(pop_member1.num_words):
            if pop_member1.attacked_text.words[i]==original_text.words[i]:
                word_pos = original_text.pos_of_word_index(i)
            else:
    ...
    def mutation(self, pop_member, original_result, index=None):
        ...
        while iterations < non_zero_indices:
            ...
            if pop_member.attacked_text.words[idx]==original_result.attacked_text.words[idx]:
                word_pos = original_result.attacked_text.pos_of_word_index(idx)
            else:
    ...
    def crossover(self, pop_member1, pop_member2, T, original_text):
        ...
        while num_tries < self.max_crossover_retries + 1:
            new_text, attributes = self.crossover_operation(pop_member1, pop_member2, T, original_text)
